...
- **The locked slot stays put.** If the dimension in the cube's *last storage position* has string elements, that position is locked — TM1's single hard storage-order rule (`CellPutS`/string writes target the last dimension), see [The Locked Slot](string-element-constraint.md). τ never proposes a swap that would move that dimension, and no dimension is ever swept *into* its slot (or an excluded dimension's). A numeric measure carries no such constraint and is free to sit wherever cardinality places it — and neither does a string-bearing dimension that is *not* last, which is placed by cardinality like any other.
